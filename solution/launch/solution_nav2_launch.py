...
def robot_controller_actions(context : LaunchContext):
    ...
    for robot_number in range(1, num_robots + 1):
        ...
        group = GroupAction([
            ...
            Node(
                package='solution',
                executable = 'robot_item_tracker',
                output='screen'
            ),
            Node(
                package='solution',
                executable = 'robot_crash_detection',
                output='screen'
            )
...
def generate_launch_description():
    ...
    repeat_colours = LaunchConfiguration('repeat_colours')
    ...
    declare_assignment_mode_cmd = DeclareLaunchArgument(
        'assignment_mode',
        default_value='cyclical',
        description='How target items will be assigned to the robots. Cyclical, Best, Single, Random'
    )

    declare_target_colour_cmd = DeclareLaunchArgument(
        'target_colour',
        default_value='red',
        description='Which colour items the robots will target in single colour mode'
    )

    declare_repeat_colours_cmd = DeclareLaunchArgument(
        'repeat_colours',
        default_value='false',
        description='Can colours be repeated when assigning targets'
    )
    ...
    #Launch Arguments doesn't appear to work, but values can be changed easily in the file
    robot_targets_cmd = Node(
        package='solution',
        executable='robot_targets',
        output='screen',
        arguments=['--mode', assignment_mode,
                   '--colour', target_colour,
                   '--repeat', repeat_colours])
    
    robot_warning_cmd = Node(
        package='solution',
        executable='robot_collision_warning',
        output='screen')
    ...
    ld.add_action(declare_assignment_mode_cmd)
    ld.add_action(declare_target_colour_cmd)
    ld.add_action(declare_repeat_colours_cmd)
    ...
    ld.add_action(robot_targets_cmd)
    ld.add_action(robot_warning_cmd)
